...
// Adapted from ClientWorker in lcclient, this version only produce the payloads then save to a file
std::vector<work_unit> ClientWorker(int c, int id, WaitGroup *starter, WaitGroup *starter2, std::function<std::vector<work_unit>()> wf) {
  srand(time(NULL) * (id + 1));
  std::vector<work_unit> w(wf());
  
  std::ofstream out(std::to_string(id) + ".bin", std::ios::binary);
  if (!out.is_open()) {
  ...
  for (const auto& unit : w) {
    ...
    printf("Finished writing requests\n");
